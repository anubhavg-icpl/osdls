...
openssl req -new -x509 -sha256 -key $CERT_DIR/root-ca-key.pem -out $CERT_DIR/root-ca.pem -days 730 -subj "/C=IN/ST=Gujarat/L=Ahmedabad/O=OPENARMOR/CN=Root CA"

# Function to generate node certificate
generate_node_cert() {
    local NODE=$1
    openssl genrsa -out $CERT_DIR/$NODE-key.pem 2048
    openssl req -new -key $CERT_DIR/$NODE-key.pem -out $CERT_DIR/$NODE.csr -subj "/C=IN/ST=Gujarat/L=Ahmedabad/O=OPENARMOR/CN=$NODE"
